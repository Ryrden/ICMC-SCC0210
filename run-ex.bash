...
if ! ls $testcaseDir/*.in 1> /dev/null 2>&1 || ! ls $testcaseDir/*.out 1> /dev/null 2>&1; then
    ...
    if [ -f "$zipfile" ]; then
        ...
        unzip -q -o $zipfile -d $testcaseDir
...
run_test() {
    ...
    if [ -z "$diff_output" ]; then
        echo -e "${GREEN}$CHECKMARK Test case $input_file passed.${NC}"
    else
        echo -e "${RED}$CROSS Test case $input_file failed.${NC}"
        echo "$diff_output"
        echo -e "${RED}Stopping further tests due to failure.${NC}"
        all_passed=false
        return 1
    fi
    echo
